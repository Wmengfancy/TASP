function [TaskComputationSpeed,AvgFinishtime] = P2_RankOnNum(Tasknum,userNum,Servernum,Cache,Possionrate,Possionrate_sum,Taskgraph,Transferrate,Computespeed_Local,ComputeSpeed_server)
...
Taskgraph_finally = Cal_Communicationtime(Taskgraph_finally,Transferrate,Cachelocation,Tasknum,userNum,Servernum,Taskgraph,Possionrate,Possionrate_sum); %处理边的权重（通信时间），边的数据量 / 两个服务器的通信速率。对于本地执行的任务，通信时间需要计算加权平均
...
for i=1:userNum
    [EFT,Lastfinishtask] = Cal_Taskfinishtime(i,Cachelocation,TaskComputationSpeed,Taskgraph,Tasknum,Servernum,Possionrate,Possionrate_sum,Computespeed_Local,Transferrate);
