function [Taskgraph,Graph] = GenarateGraphCommon_2(Graph,Tasknum,userNum,commonration)
...
for k=1:userNum
    ...
    %新增代码
    NodeSet = zeros(1,0); %第k个用户DAG含有的子任务集合
    for i=1:Tasknum
    ...
    firstTaskId = NodeSet(1);
    [first_root,f] = findRoot(f,firstTaskId);
    
    for index = 2:ContainTaskNum(k)
        curTaskId = NodeSet(index);
         [root_cur,f] = findRoot(f,curTaskId);
         if(root_cur ~= first_root)
             f(root_cur) = first_root;
             %在任务NodeSet(index - 1)与curTaskId之间添加一条边
              Taskgraph(NodeSet(index - 1),curTaskId,k) = 1;
              Taskgraph(curTaskId,NodeSet(index - 1),k) = -1;
              Graph(NodeSet(index - 1),curTaskId) = 1;
              Graph(curTaskId,NodeSet(index - 1)) = -1;
         end
    end
